...
code = requests.get("http://www.cgv.co.kr/movies/?lt=1&ft=0") # cgv 영화 메인 홈페이지
soup = BeautifulSoup(code.text, "html.parser")                # 파싱하기

title_elements = soup.select("strong.title")             #영화 제목       
image_elements = soup.select("span.thumb-image > img")   #이미지
percent_elements = soup.select("strong.percent > span")  #예매율
date_elements = soup.select("span.txt-info > strong")    #개봉날짜
